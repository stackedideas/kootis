...
const FILTERS = ["All", "Totes", "Clutches", "Crossbody", "Satchels"];
...
const hero = (
  ...
  >
    ...
    <nav className="flex items-center gap-2 font-sans" style={{ fontSize: "12px" }}>
      ...
      <span className="text-white">Handbags</span>
    </nav>
  </section>
);

export default function BodyShopHandbags() {
  const { products, loading, error } = useProducts();
  const bags = products.filter((p) => BAG_CATS.includes(p.category));
  return <CategoryPage hero={hero} filters={FILTERS} products={bags} loading={loading} error={error} />;
}
